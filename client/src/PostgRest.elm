...
-}
list : Limit -> String -> List Http.Header -> Query uniq schema a -> Http.Request (List a)
list limit url authHeaders (Query _ (Parameters params) decoder) =
    let
        settings =
            { count = False
            , singular = False
...
-}
first : String -> List Http.Header -> Query uniq schema a -> Http.Request (Maybe a)
first url authHeaders (Query _ (Parameters params) decoder) =
    let
        settings =
            { count = False
            , singular = True
...
{-| -}
paginate : { pageNumber : Int, pageSize : Int } -> String -> List Http.Header -> Query uniq schema a -> Http.Request (Page a)
paginate { pageNumber, pageSize } url authHeaders (Query _ (Parameters params) decoder) =
    let
        settings =
